<cfcomponent>
	...
	<cffunction name="init" returntype="lib.buildStruct" access="public" output="no">
		...
		<cfset setData(arguments.payload)>
		<cfset setPayload(arguments.payload)>
	...
	<!--- DUMP FUNCTION (this is what I do in the restroom) --->
	<cffunction name="dump" access="public" returntype="struct" output="false" >
		<cfreturn variables.instance />
	</cffunction>
	
    <!--- JSON CONVERSION --->
	<cffunction name="getJSON" access="public" returntype="any" output="false" >
		<cfreturn jsonencode(data=variables.instance,formatDates=true) />
	</cffunction>
	...
	<cffunction name="setData" access="public" returntype="void" output="false">
		<cfargument name="dataset" type="any" required="true" />
		<cfset variables.instance.dataset = arguments.dataset />
    ...
    <!--- STATUS --->
	<cffunction name="getStatus" access="public" returntype="boolean" output="false">
	...
	<cffunction name="jsonencode" access="remote" returntype="string" output="No"
		...
		<cfelseif IsDate(_data) AND arguments.formatDates>
			<cfreturn '"#DateFormat(_data, "YYYY-MM-DD")#T#TimeFormat(_data, "HH:MM:SS")#Z"' />
