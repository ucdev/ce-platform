...
ce.module("activity.participants",function(self,ce,Backbone,Marionette,$,_) {
	...
	self.getParticipants = function() {
		$.ajax({
			...
			async: true,
			success: function(data) {
				self.details.totalAttendeeList['attendees'] = {} = data.DATASET;
				self.rows = data.PAYLOAD;
	...
	self.loadRegistrants = function() {
		...
		$('body').on('data_loaded', function() {
			...
			attendeesStatusList = self.details.totalAttendeeList['statuses'];
			
			// CREATE EACH PARTICIPANT ROW
			$.each(self.rows, function(id, item) {
				var data = item.data;
				...
				self.rows[id] = new ce.Models.Activity_participant(data);
				
				// PROVIDE PARTICIPANT ROW LOGIC TO CURRENT ATTENDEE
				self.rows[id].view = new self.RowView({ model: self.rows[id] });
				
				updateRowView = self.RowView.extend({
					initialize : function(options) {
						this.render = _.bind(this.render, this);
						
						this.model.bind('change:fullName', this.render);
					}
				});
			});
			...
			//self.pager.updatePaginator();
			
			// UPDATE VIEWABLE ATTENDEE ROWS
			//self.pager.changePage();
