<cfcomponent
	output="false"
	hint="I provide a wrapper to the SendGrid Web API (http://sendgrid.com).">
	...
	<cffunction
		...
		<cfargument
			...
			type="string"
			required="true"
			hint="This is the same credential used for your SMTP settings, and for logging into the website."
			/>
		...
		<cfargument
			...
			type="string"
			required="true"
			hint="This is the same password to authenticate over SMTP, and for logging into the website."
		/>
			
		<!--- Store the API key. --->
		<cfset variables.apiUser = arguments.apiUser />
		<!--- Store the API key. --->
		<cfset variables.apiKey = arguments.apiKey />
		
		<!--- Store the base URL for the v1.0 API. --->
		<cfset variables.apiBaseUrl = "https://sendgrid.com/api/" />
	...
	<cffunction name="X-SMTPAPI" access="public" output="false" returntype="string">
