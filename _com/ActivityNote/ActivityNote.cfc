<cfcomponent displayname="ActivityNote" output="false">
	<!---
	PROPERTIES
	--->
	...
	--->
	<cffunction name="init" access="public" returntype="_com.ActivityNote.ActivityNote" output="false">
		<cfargument name="NoteID" type="string" required="false" default="" />
		...
		<!--- run setters --->
		<cfset setNoteID(arguments.NoteID) />
	...
	PUBLIC FUNCTIONS
	--->
	<cffunction name="setMemento" access="public" returntype="_com.ActivityNote.ActivityNote" output="false">
	...
	<cffunction name="validate" access="public" returntype="array" output="false">
		...
		<!--- NoteID --->
		<cfif (len(trim(getNoteID())) AND NOT isNumeric(trim(getNoteID())))>
			<cfset thisError.field = "NoteID" />
			<cfset thisError.type = "invalidType" />
			<cfset thisError.message = "NoteID is not numeric" />
		...
		<cfif (len(trim(getBody())) GT 63000)>
			<cfset thisError.field = "Body" />
	...
	ACCESSORS
	--->
	<cffunction name="setNoteID" access="public" returntype="void" output="false">
		<cfargument name="NoteID" type="string" required="true" />
		<cfset variables.instance.NoteID = arguments.NoteID />
	</cffunction>
	<cffunction name="getNoteID" access="public" returntype="string" output="false">
