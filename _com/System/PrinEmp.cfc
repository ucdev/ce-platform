<cfcomponent displayname="PrinEmp" output="false">
	<!---
	PROPERTIES
	--->
	...
	--->
	<cffunction name="init" access="public" returntype="_com.System.PrinEmp" output="false">
	...
	PUBLIC FUNCTIONS
	--->
	<cffunction name="setMemento" access="public" returntype="_com.System.PrinEmp" output="false">
	...
	<cffunction name="validate" access="public" returntype="array" output="false">
		...
		<!--- Created --->
		<cfif (len(trim(getCreated())) AND NOT IsSimpleValue(trim(getCreated())))>
			<cfset thisError.field = "Created" />
			<cfset thisError.type = "invalidType" />
			<cfset thisError.message = "Created is not a string" />
			<cfset arrayAppend(errors,duplicate(thisError)) />
		</cfif>
		<cfif (len(trim(getCreated())) GT 10)>
			<cfset thisError.field = "Created" />
			<cfset thisError.type = "tooLong" />
			<cfset thisError.message = "Created is too long" />
			<cfset arrayAppend(errors,duplicate(thisError)) />
		</cfif>
