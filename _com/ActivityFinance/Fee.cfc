...
<cfcomponent displayname="Fee" output="false">
	<!---
	PROPERTIES
	--->
	...
	--->
	<cffunction name="init" access="public" returntype="_com.ActivityFinance.Fee" output="false">
		<cfargument name="FeeID" type="string" required="false" default="" />
		...
		<!--- run setters --->
		<cfset setFeeID(arguments.FeeID) />
	...
	PUBLIC FUNCTIONS
	--->
	<cffunction name="setMemento" access="public" returntype="_com.ActivityFinance.Fee" output="false">
	...
	<cffunction name="validate" access="public" returntype="array" output="false">
		...
		<!--- FeeID --->
		<cfif (len(trim(getFeeID())) AND NOT isNumeric(trim(getFeeID())))>
			<cfset thisError.field = "FeeID" />
			<cfset thisError.type = "invalidType" />
			<cfset thisError.message = "FeeID is not numeric" />
		...
		<cfif (len(trim(getDisplayName())) GT 300)>
			<cfset thisError.field = "DisplayName" />
			<cfset thisError.type = "tooLong" />
	...
	ACCESSORS
	--->
	<cffunction name="setFeeID" access="public" returntype="void" output="false">
		<cfargument name="FeeID" type="string" required="true" />
		<cfset variables.instance.FeeID = arguments.FeeID />
	</cffunction>
	<cffunction name="getFeeID" access="public" returntype="string" output="false">
		<cfreturn variables.instance.FeeID />
	</cffunction>
