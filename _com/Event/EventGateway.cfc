<cfcomponent displayname="EventGateway">
	<cffunction name="init" access="public" output="false" returntype="_com.Event.EventGateway">
	...
	<cffunction
		...
		--->
		<cfquery name="LOCAL.RawEvent" datasource="#variables.DSN#">
			SELECT     
				id, 
				name, 
			...
			WHERE
				...
				(
						date_ended >= <cfqueryparam value="#ARGUMENTS.From#" cfsqltype="CF_SQL_TIMESTAMP" /><cfif Arguments.ActivityID NEQ ""> AND ActivityID=<cfqueryparam value="#Arguments.ActivityID#" cfsqltype="cf_sql_integer" /></cfif>
					OR
						date_ended IS NULL	<cfif Arguments.ActivityID NEQ ""> AND ActivityID=<cfqueryparam value="#Arguments.ActivityID#" cfsqltype="cf_sql_integer" /></cfif>
				)
			AND
				<!--- Check start date constraints. --->
				date_started <= <cfqueryparam value="#ARGUMENTS.To#" cfsqltype="CF_SQL_TIMESTAMP" /><cfif Arguments.ActivityID NEQ ""> AND ActivityID=<cfqueryparam value="#Arguments.ActivityID#" cfsqltype="cf_sql_integer" /></cfif>
		</cfquery>
		...
		--->
		<cfquery name="LOCAL.EventException" datasource="#variables.DSN#">
			SELECT
				e.[date],
				e.event_id
			FROM
				ce_Event_Exception e
			WHERE
